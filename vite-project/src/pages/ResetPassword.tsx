...
import { TextField, Button, CircularProgress, InputAdornment, IconButton, Typography } from "@mui/material";
...
const Schema = Yup.object({
  ...
  confirmPassword: Yup.string().oneOf([Yup.ref("password")], "Passwords must match").required("Confirm required"),
});
    ...
    <AuthLayout title="Reset Password" subtitle="Set a new password for your account">
      <Formik
        ...
        onSubmit={async (values) => {
          ...
          const res: any = await dispatch(resetPasswordThunk({ resetToken, password: values.password }));
      ...
      >
        {({ values, handleChange, errors, touched, isSubmitting }) => (
          <Form className="space-y-4">
            <TextField
              fullWidth label="New Password" name="password" type={show ? "text" : "password"}
            ...
            <TextField fullWidth label="Confirm Password" name="confirmPassword" type="password"
              ...
              error={touched.confirmPassword && Boolean(errors.confirmPassword)} helperText={touched.confirmPassword && errors.confirmPassword} />
